...
API_URL = 'http://543d2db2.ngrok.io/raw_data' # PokemonGo Map from PokemonGoMap
PAGE_ACCESS_TOKEN = 'EAATEPxKykMUBAC0fgIkgn5ogcsehZCWANJ9yUCr4U5lpRMPCHX8rLBnUgX1sD0xchi2UNGnnEXSIyQjsCujF1J99Mkp32XCyYQ3XLDB0DzeaceZBy4NvHmXydvxGzN7z31AnqgHhbfeC5W9DBEmj9PmU9uiGfU7DWAbZAhUH7J5dLgxI0J1' # Facebook page api token
VERIFY_TOKEN = 'pokegoroverde'
post_message_url = 'https://graph.facebook.com/v2.7/me/messages?access_token=' + PAGE_ACCESS_TOKEN


vegan_mode = {}
last_called = {}
shit_list = set(['Pidgey', 'Rattata', 'Weedle'])

rarity_list = [set(['Caterpie','Zubat','Ekans','Sandshrew','Nidoran','Geodude','Poliwag','Mankey','Venonat','Paras','Gastly','Krabby','Voltorb','Goldeen','Magikarp']),
               set(['Bulbasaur','Slowpoke','Raticate','Pidgeotto','Drowzee','Spearow','Bellsprout','Horsea','Charmander','Metapod','Kakuna','Oddish','Arbok','Psyduck','Persian','Diglet','Golbat','Meowth','Jigglypuff','Vulpix','Magnemite','Clefairy','Sandslash','Pikachu','Growlith','Machoke','Tentacool','Graveler','Ponyta','Magneton','Doduo','Koffing','Hitmonlee','Cubone','Exeggcute','Electrode','Drowzee','Haunter','Shellder','Grimer','Rhyhorn','Staryu','Jynx']),
...
def webhook():
    if request.method == 'POST':
        ...
        if data is None:
            payload = json.dumps({'recipient': {'id': sender}, 'message': {'text': 'El servidor del mapa debe estar caido.. Informando a los administradores'}})
            ...
            for message_entry in entry['messaging']:
                ...
                if 'message' in message_entry and 'text' in message_entry['message']:
                    ...
                    if 'fuck' in text.lower() or 'shit' in text.lower() or 'cunt' in text.lower():
                        ...
                        payload = json.dumps({'recipient': {'id': sender}, 'message': {'text': 'Comandos disponibles: \nLista - Muestra en pantalla los pokemon cercanos \nRadar - Modo radar \nBTW DON\'T SWEAR AT ME'}})
                        status = requests.post(post_message_url, headers={"Content-Type": "application/json"}, data=payload)    
                    elif 'radar' in text.lower():
                        if sender not in vegan_mode:
                            vegan_mode[sender] = True
                            payload = json.dumps({'recipient': {'id': sender}, 'message': {'text': 'Modo Radar Activado...'}})
                            status = requests.post(post_message_url, headers={"Content-Type": "application/json"}, data=payload)
                            continue
                        ...
                        if not vegan_mode[sender]:
                            ...
                            payload = json.dumps({'recipient': {'id': sender}, 'message': {'text': 'Modo Radar Desactivado...'}})
                            status = requests.post(post_message_url, headers={"Content-Type": "application/json"}, data=payload)
                        ...
                        if (sender in last_called and (message_entry['timestamp'] - last_called[sender] ) < 30000): # 30 seconds cooldown
                            ...
                            payload = json.dumps({'recipient': {'id': sender}, 'message': {'text': 'Perdon, estas unos %s segundos en espera' % str(cool_down)}})
                            status = requests.post(post_message_url, headers={"Content-Type": "application/json"}, data=payload)
                            continue
                        ...
                        payload = json.dumps({'recipient': {'id': sender}, 'message': {'text':'ATRÁPALOS A TODOS!!!'}})
